...
export const StageCard: React.FC<StageCardProps> = ({ stage, isActive, onClick }) => {
  ...
  useEffect(() => {
    if (isActive && contentRef.current) {
      // Optional: Scroll into view if needed, but automatic scrolling can be jarring
  ...
  return (
    <div 
      ...
      className={`
        ...
        ${isActive 
          ? 'bg-slate-900/80 border-ksa-gold shadow-[0_0_30px_rgba(197,160,89,0.15)] scale-[1.02] z-10 ring-1 ring-ksa-gold/50' 
    ...
    >
      <div className="flex justify-between items-start mb-3">
        <div>
          <div className="flex items-center gap-2">
             <h3 className={`font-display text-xl font-semibold tracking-wide ${isActive ? 'text-ksa-gold' : 'text-slate-200 group-hover:text-slate-100'}`}>
               {stage.stageName}
             </h3>
        ...
        <div className={`
          flex items-center justify-center w-10 h-10 rounded-full font-bold text-sm border backdrop-blur-sm transition-colors duration-300
        ...
        <div className="overflow-hidden">
          ...
          <div className="space-y-2.5">
            {stage.experiencePoints.map((exp, idx) => (
              <div key={idx} className="flex items-start gap-3 p-2.5 rounded-lg bg-black/20 border border-white/5 hover:border-white/10 transition-colors">
                <div className={`
                ...
                <div>
                  <p className="text-sm font-medium text-slate-200 leading-tight mb-0.5">{exp.title}</p>
      ...
      {!isActive && (
         <div className="mt-3 text-[10px] text-slate-600 text-center uppercase tracking-widest font-medium flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
